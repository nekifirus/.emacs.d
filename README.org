...
* Email
   #+begin_src emacs-lisp
     ...
     (use-package-ensure-elpa 'org-mu4e
                              '(nil)
                              'nil)
     (defvar use-package--warning66
       #'(lambda
           (keyword err)
           (let
               ((msg
                 (format "%s/%s: %s" 'org-mu4e keyword
         ...
         (progn
           (let
             ...
             (custom-theme-set-variables 'use-package
                                         '(org-mu4e-link-query-in-headers-mode nil nil nil "Customized with use-package org-mu4e")))
           (let
               ((now
                 (current-time)))
             (message "%s..." "Loading package org-mu4e")
             (prog1
                 (if
                     (not
                      (require 'org-mu4e nil t))
                     (display-warning 'use-package
                                      (format "Cannot load %s" 'org-mu4e)
                                      :error)
                   (unless
                       (use-package-ensure-system-package-exists\? 'mu)
               ...
               (let
                 ...
                 (if
                     (> elapsed 0.005)
                     (message "%s...done (%.3fs)" "Loading package org-mu4e" elapsed)
                   (message "%s...done" "Loading package org-mu4e"))))))
       (error
        (funcall use-package--warning66 :catch err))))
   #+end_src
* Telega
#+begin_src emacs-lisp
  (use-package telega
