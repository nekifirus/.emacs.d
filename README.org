...
#+begin_src emacs-lisp
  (use-package claude-code
    ...
    ;; Фикс: после нажатия return буфер прокручивался в начало display-области.
    ;; Причина: Claude CLI перерисовывает TUI, курсор кратко попадает в начало
    ;; display-области, eat следует за ним. Переопределяем функцию синхронизации
    ;; скролла так, чтобы прокрутка срабатывала только если курсор во второй
    ;; половине буфера (т.е. это новый вывод, а не артефакт перерисовки TUI).
    (defun claude-code--eat-synchronize-scroll (windows)
      "Синхронизация скролла eat без прыжков в начало при перерисовке TUI."
              ...
              (cond
               ((>= cursor-pos (- (point-max) 2))
                (with-selected-window window
                  ...
                  (recenter -1)))
               ((and (not (pos-visible-in-window-p cursor-pos window))
                     (> cursor-pos (/ (point-max) 2)))
                (with-selected-window window
                  (goto-char cursor-pos)
                  (recenter)))))))))
